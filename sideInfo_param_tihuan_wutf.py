...
from utils import *
from data_model_wutf import DataModel
...
class SideInfo(object):
    ...
    def process(self):
        ...
        if self.p.use_Entity_linking_dict:
            ...
            # fname3, fname4 = self.folder_to_make + '/new_ent_list', self.folder_to_make + '/new_rel_list'
            # fname5 = self.folder_to_make + '/new_trpIds'
            # fname6, fname7 = self.folder_to_make + '/new_ent2id', self.folder_to_make + '/new_rel2id'
            # fname8, fname9 = self.folder_to_make + '/new_id2ent', self.folder_to_make + '/new_id2rel'
            ...
            if not checkFile(fname1) or self.p.change_EL_threshold:
                ...
                # print('old_ent:', len(self.ent_list))
                # print('old_rel:', len(self.rel_list))
                # new_ent_list, new_rel_list = [], []
                #
                # for triple in self.triples:
                #     trp = (self.ent_old_id2new_id[self.ent2id[triple['triple'][0]]],
                #            self.rel_old_id2new_id[self.rel2id[triple['triple'][1]]],
                #            self.ent_old_id2new_id[self.ent2id[triple['triple'][2]]])
                #     self.new_trpIds.append(trp)
                    # new_ent_list.append(self.id2ent[self.ent_old_id2new_id[self.ent2id[triple['triple'][0]]]])
                    # new_rel_list.append(self.id2rel[self.rel_old_id2new_id[self.rel2id[triple['triple'][1]]]])
                    # new_ent_list.append(self.id2ent[self.ent_old_id2new_id[self.ent2id[triple['triple'][2]]]])
                #
                # self.new_ent_list, self.new_rel_list = list(set(new_ent_list)), list(set(new_rel_list))
                # print('new_ent:', len(self.new_ent_list))
                # print('new_rel:', len(self.new_rel_list))
                #
                # Generate a unique id for each entity and relations
                # self.new_ent2id = dict([(v, k) for k, v in enumerate(self.new_ent_list)])
                # self.new_rel2id = dict([(v, k) for k, v in enumerate(self.new_rel_list)])
                # self.new_id2ent = invertDic(self.new_ent2id)
                # self.new_id2rel = invertDic(self.new_rel2id)
                #
                # for triple in self.triples:
                #     trp = (self.new_ent2id[self.id2ent[self.ent_old_id2new_id[self.ent2id[triple['triple'][0]]]]],
                #            self.new_rel2id[self.id2rel[self.rel_old_id2new_id[self.rel2id[triple['triple'][1]]]]],
                #            self.new_ent2id[self.id2ent[self.ent_old_id2new_id[self.ent2id[triple['triple'][2]]]]])
                #     self.new_trpIds.append(trp)

                pickle.dump(self.ent_old_id2new_id, open(fname1, 'wb'))
                pickle.dump(self.rel_old_id2new_id, open(fname2, 'wb'))
                # pickle.dump(self.new_ent_list, open(fname3, 'wb'))
                # pickle.dump(self.new_rel_list, open(fname4, 'wb'))
                # pickle.dump(self.new_trpIds, open(fname5, 'wb'))
                # pickle.dump(self.new_ent2id, open(fname6, 'wb'))
                # pickle.dump(self.new_rel2id, open(fname7, 'wb'))
                # pickle.dump(self.new_id2ent, open(fname8, 'wb'))
                # pickle.dump(self.new_id2rel, open(fname9, 'wb'))
            else:
                ...
                # self.new_ent_list = pickle.load(open(fname3, 'rb'))
                # self.new_rel_list = pickle.load(open(fname4, 'rb'))
                # self.new_trpIds = pickle.load(open(fname5, 'rb'))
                # self.new_ent2id = pickle.load(open(fname6, 'rb'))
                # self.new_rel2id = pickle.load(open(fname7, 'rb'))
                # self.new_id2ent = pickle.load(open(fname8, 'rb'))
                # self.new_id2rel = pickle.load(open(fname9, 'rb'))

            # fname1, fname2 = self.folder_to_make + '/entity_seed_pair_EL_list', self.folder_to_make + '/relation_seed_pair_EL_list'
            # if not checkFile(fname1) or not checkFile(fname2) or self.p.change_EL_threshold:
            #     print('generate seed_pair_Entity_linking_list')
            #     self.entity_seed_pair_EL_list, self.relation_seed_pair_EL_list = self.seed_pair_Entity_linking_list()
            #     pickle.dump(self.entity_seed_pair_EL_list, open(fname1, 'wb'))
            #     pickle.dump(self.relation_seed_pair_EL_list, open(fname2, 'wb'))
            # else:
            #     print('load seed_pair_Entity_linking_list')
            #     self.entity_seed_pair_EL_list = pickle.load(open(fname1, 'rb'))
            #     self.relation_seed_pair_EL_list = pickle.load(open(fname2, 'rb'))
            #
            # if self.p.change_EL_threshold:
            #     all_seed_pair, true_seed_pair = 0, 0
            #     for i in range(len(self.entity_seed_pair_EL_list)):
            #         all_seed_pair += 1
            #         line = self.entity_seed_pair_EL_list[i]
            #         mention1, mention2 = line[0], line[1]
            #         true_link1, true_link2 = str(self.ent2true_link[str(mention1)]), str(
            #             self.ent2true_link[str(mention2)])
            #         if true_link1 == true_link2:
            #             true_seed_pair += 1
            #     print('true_seed_pair:', true_seed_pair)
            #     print('all_seed_pair:', all_seed_pair)
            #     print('precision:', true_seed_pair / all_seed_pair)
    ...
    def seed_pair_Entity_linking_list(self):
        self.entity_seed_pair_EL_list, self.relation_seed_pair_EL_list = [], []
        for entity in self.entity_score_EL_dict.keys():
            mention = self.entity_score_EL_dict[entity]
            iter = list(itertools.combinations(mention, 2))
            if len(iter) > 0:
                for i in range(len(iter)):
                    pair = iter[i]
                    self.entity_seed_pair_EL_list.append(pair)
        for entity in self.relation_score_EL_dict.keys():
            mention = self.relation_score_EL_dict[entity]
            iter = list(itertools.combinations(mention, 2))
            if len(iter) > 0:
                for i in range(len(iter)):
                    pair = iter[i]
                    self.relation_seed_pair_EL_list.append(pair)
        print('self.entity_seed_pair_EL_list:', len(self.entity_seed_pair_EL_list))
        print('self.relation_seed_pair_EL_list:', len(self.relation_seed_pair_EL_list))
        return self.entity_seed_pair_EL_list, self.relation_seed_pair_EL_list

    def nameEmbedding(self):
        self.seed_pair, self.seed, self.seed_sim = [], [], []
        use_el = self.p.use_Entity_linking_dict
        print('self.p.use_Entity_linking_dict:', type(self.p.use_Entity_linking_dict), self.p.use_Entity_linking_dict)
        print('use_el:', type(use_el), use_el)
        print('------------------------------------------------------------------------------------------------')
        if use_el:
            entity_threshold = self.p.entity_threshold
            for id1 in range(0, len(self.new_id2ent.keys())):
                for id2 in range(id1 + 1, len(self.new_id2ent.keys())):
                    ent1, ent2 = self.new_id2ent[id1], self.new_id2ent[id2]
                    if ent1 == ent2:
                        print('ent1 == ent2, so continue')
                        continue
                    if (ent1, ent2) in self.seed_pair or (ent2, ent1) in self.seed_pair:
                        print('had in seed pair, so continue')
                        continue
                    if not np.dot(self.ent2embed_name[id1], self.ent2embed_name[id2]) == 0:
                        sim_name = cos_sim(self.ent2embed_name[id1], self.ent2embed_name[id2])
                        if sim_name > entity_threshold:
                            seed_pair, sim = (ent1, ent2), sim_name
                            self.seed_pair.append(seed_pair)
                            for triple in self.new_trpIds:
                                if str(self.new_id2ent[triple[0]]) == str(ent1):
                                    trp = (self.new_ent2id[str(ent2)], triple[1], triple[2])
                                    self.seed_trpIds.append(trp)
                                    self.seed_sim.append(sim)
                                if str(self.new_id2ent[triple[0]]) == str(ent2):
                                    trp = (self.new_ent2id[str(ent1)], triple[1], triple[2])
                                    self.seed_trpIds.append(trp)
                                    self.seed_sim.append(sim)
                                if str(self.new_id2ent[triple[2]]) == str(ent1):
                                    trp = (triple[0], triple[1], self.new_ent2id[str(ent2)])
                                    self.seed_trpIds.append(trp)
                                    self.seed_sim.append(sim)
                                if str(self.new_id2ent[triple[2]]) == str(ent2):
                                    trp = (triple[0], triple[1], self.new_ent2id[str(ent1)])
                                    self.seed_trpIds.append(trp)
                                    self.seed_sim.append(sim)
                                # trp = (self.ent2id[triple['triple'][0]], self.rel2id[triple['triple'][1]],
                                # self.ent2id[triple['triple'][2]])
                                # self.seed_trpIds.append(trp)
                            # self.ent2name_seed[(ent1, ent2)] = sim_name
            print('entity seed is ok ')
            print('number of entity seed pair:', len(self.seed_pair), type(self.seed_pair))
            print('len(self.seed_sim):', len(self.seed_sim), type(self.seed_sim))
            print('len(self.seed_trpIds):', len(self.seed_trpIds), type(self.seed_trpIds))

            relation_threshold = self.p.relation_threshold
            for id1 in range(0, len(self.new_id2rel.keys())):
                for id2 in range(id1 + 1, len(self.new_id2rel.keys())):
                    rel1, rel2 = self.new_id2rel[id1], self.new_id2rel[id2]
                    if rel1 == rel2:
                        print('rel1 == rel2, so continue')
                        continue
                    if (rel1, rel2) in self.seed_pair or (rel2, rel1) in self.seed_pair:
                        print('had in seed pair, so continue')
                        continue
                    if not np.dot(self.rel2embed_name[id1], self.rel2embed_name[id2]) == 0:
                        sim_name = cos_sim(self.rel2embed_name[id1], self.rel2embed_name[id2])
                        if sim_name > relation_threshold:
                            seed_pair, sim = (rel1, rel2), sim_name
                            self.seed_pair.append(seed_pair)
                            for triple in self.new_trpIds:
                                if str(self.new_id2rel[triple[1]]) == str(rel1):
                                    trp = (triple[0], self.new_rel2id[str(rel2)], triple[2])
                                    self.seed_trpIds.append(trp)
                                    self.seed_sim.append(sim)
                                if str(self.new_id2rel[triple[1]]) == str(rel2):
                                    trp = (triple[0], self.new_rel2id[str(rel1)], triple[2])
                                    self.seed_trpIds.append(trp)
                                    self.seed_sim.append(sim)

            print('number of relation seed pair:', len(self.seed_pair), type(self.seed_pair))
            print('all seed is ok ')
            print('len(self.seed_sim):', len(self.seed_sim), type(self.seed_sim))
            print('len(self.seed_trpIds):', len(self.seed_trpIds), type(self.seed_trpIds))
        else:
            entity_threshold = self.p.entity_threshold
            for id1 in range(0, len(self.id2ent.keys())):
                for id2 in range(id1 + 1, len(self.id2ent.keys())):
                    ent1, ent2 = self.id2ent[id1], self.id2ent[id2]
                    if ent1 == ent2:
                        print('ent1 == ent2, so continue')
                        continue
                    if (ent1, ent2) in self.seed_pair or (ent2, ent1) in self.seed_pair:
            ...
            relation_threshold = self.p.relation_threshold
            for id1 in range(0, len(self.id2rel.keys())):
                for id2 in range(id1 + 1, len(self.id2rel.keys())):
                    rel1, rel2 = self.id2rel[id1], self.id2rel[id2]
                    if rel1 == rel2:
                        print('rel1 == rel2, so continue')
                        continue
                    if (rel1, rel2) in self.seed_pair or (rel2, rel1) in self.seed_pair:
            ...
            print('len(self.seed_sim):', len(self.seed_sim), type(self.seed_sim))
            print('len(self.seed_trpIds):', len(self.seed_trpIds), type(self.seed_trpIds))
        return self.seed_pair, self.seed_sim, self.seed_trpIds
    ...
    def get_EL_seed(self):
        ...
        # entity_seed_length: 193293 relation_seed_length: 4439 all_seed_length: 197732
