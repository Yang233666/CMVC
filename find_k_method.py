...
from utils import cos_sim, cosine_distance, normalize
...
class Inverse_JumpsMethod(object):
    ...
    def Distortions(self, random_state=0):
        ...
        for i in tqdm(cluster_range):
            ...
            # # metric = 'seuclidean'
            # # metric = 'sqeuclidean'
            # dist = pdist(self.data, metric=metric)
            # method = 'single'
            # # method = 'complete'
            # # method = 'average'
            # # method = 'weighted'
            ...
            # # method = 'median'
            # # method = 'ward'
            # clust_res = linkage(dist, method=method)
            # labels = fcluster(clust_res, t=parameter, criterion='maxclust') - 1
            # clusters = [[] for i in range(max(labels) + 1)]
            # for j in range(len(labels)):
            #     clusters[labels[j]].append(j)
            # centers = np.zeros((len(clusters), self.p), np.float32)
            # parameter = centers.shape[0]
            # for j in range(len(clusters)):
            #     cluster = clusters[j]
            #     clusters_center_embed = np.zeros(self.p, np.float32)
            #     for m in cluster:
            #         embed_ = self.data[m]
            #         clusters_center_embed += embed_
            #     clusters_center_embed_ = clusters_center_embed / len(cluster)
            #     centers[j, :] = clusters_center_embed_
            ...
            for j in range(len(self.data)):
                ...
                for cluster in range(K):
                    ...
                    # dists[cluster] = euclidean(normalize(self.data[j]), normalize(centers[cluster]))
                    euclidean_d = euclidean(normalize(self.data[j]), normalize(centers[cluster]))
                    dists[cluster] = euclidean_d * euclidean_d / 2
                    # dists[cluster] = cosine_distance(self.data[j], centers[cluster])
        ...
        # print('self.distortions:', type(self.distortions), self.distortions.shape, self.distortions)
    ...
    def Jumps(self, distortions=None):
        ...
        # self.jumps = [np.log(self.distortions[0]) - 0]
        self.jumps = []
        ...
        # self.jumps += [np.abs(np.log(self.distortions[k]) - np.log(self.distortions[k - 1])) \
        #                for k in range(1, len(self.distortions))]  # argmin
        # self.jumps += [-(np.log(self.distortions[k]) - np.log(self.distortions[k - 1])) \
        #                for k in range(1, len(self.distortions))]  # argmin
        # self.jumps += [(np.log(self.distortions[k-1]) - np.log(self.distortions[k])) \
        #                for k in range(1, len(self.distortions))]  # argmin
        print('self.jumps:', type(self.jumps), len(self.jumps), self.jumps)
        # self.jumps = softmax(self.jumps)
        # print('self.jumps:', type(self.jumps), len(self.jumps), self.jumps)

        # calculate recommended number of clusters
        recommended_index = int(np.argmax(np.array(self.jumps)))
        # recommended_index = int(np.argmin(np.array(self.jumps)))
        # print('recommended_index:', recommended_index)
