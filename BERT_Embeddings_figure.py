...
class BERT_Model(object):
    ...
    def fine_tune(self):
        ...
        fname1 = '../file/' + self.p.dataset + '_' + self.p.split + '/' + folder + '/bert_cls_el_' + str(self.model_training_time) + '_' + str(self.BERT_self_training_time) + '_fig' # for 1
        ...
        if not checkFile(fname1):
            ...
            # save_path = '../file/' + self.p.dataset + '_' + self.p.split + '/' + 'multi_view/context_view/figure/'
            # step_list_name = save_path + 'step_list_'+ str(self.BERT_self_training_time) + '.npy'
            # step_list = np.load(step_list_name)
            # loss_list_name = save_path + 'loss_list_'+ str(self.BERT_self_training_time) + '.npy'
            # loss_list = np.load(loss_list_name)
            # ave_f1_list_name = save_path + 'ave_f1_list_'+ str(self.BERT_self_training_time) + '.npy'
            # ave_f1_list = np.load(ave_f1_list_name)
            # print('step_list:', type(step_list), len(step_list), step_list)
            # print('loss_list:', type(loss_list), len(loss_list), loss_list)
            ...
            for epoch in range(self.epochs):
                ...
                #     print('output_label:', type(output_label), len(output_label), output_label)
                #     clusters_list = list(output_label)
                #     # print('clusters_list:', type(clusters_list), len(clusters_list), clusters_list)
                ...
                #         cluster_test(self.p, self.side_info, clusters_list, self.true_ent2clust,
                #                  self.true_clust2ent,
                #                  print_or_not=False)
                #     print('ave_f1:', ave_f1)
                    # exit()
            ...
            # np.save(os.path.join(save_path, 'step_list_'+ str(self.BERT_self_training_time)), step_list)
            # np.save(os.path.join(save_path, 'loss_list_'+ str(self.BERT_self_training_time)), loss_list)
            # np.save(os.path.join(save_path, 'ave_f1_list_'+ str(self.BERT_self_training_time)), ave_f1_list)
