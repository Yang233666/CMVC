...
from find_k_method import Inverse_JumpsMethod, JumpsMethod, last_major_leap, last_leap, \
...
# dataset = 'data_banknote_authentication' # no
# dataset = 'digits' # no
# dataset = 'olivetti_faces' # no
# dataset = 'sonar' # no
...
if dataset == 'data_banknote_authentication': # 2
    fname = '../file/real_dataset/data_banknote_authentication.txt'
    print('load :', fname)
    data_x_and_y = np.loadtxt(fname, delimiter=',')  # delimiter参数依据原始文本数据每行数字之间符号，这里为\t
    print('data_x_and_y:', type(data_x_and_y), data_x_and_y.shape, data_x_and_y)
    input_data, labels = np.split(data_x_and_y, [4], axis=1)  # data为拆分的数组，[1]为拆分的行号或列号，axis表明按列或者行进行拆分（默认为0，即按行拆分）
    # level_one_min, level_one_max, level_one_gap = 1, 6, 1
    # level_one_min, level_one_max, level_one_gap = 1, 18, 1
    # 3-1, 4-1, 5-3, 6-3, 7-3, 8-3, 9-7, 10-7， 18-10
    labels_list = []
    for label_list in labels.tolist():
        labels_list.append(label_list[0])
    cluster_num = len(list(set(labels_list)))

elif dataset == 'echocardiogram':  # 3
    fname = '../file/real_dataset/echocardiogram.data'
    print('load :', fname)
    df = pd.read_csv(fname, header=None, error_bad_lines=False)
    df = df.to_numpy()
    # print('df:', type(df), df.shape, df)
    ...
    # print('labels_list:', type(labels_list), len(labels_list), labels_list)
    cluster_num = len(list(set(labels_list)))
    # level_one_min, level_one_max, level_one_gap = 1, 5, 1
    # 3-1, 4-2, 5-3, 6-4, 7-5, 8-5, 9-7, 10-7

elif dataset == 'iris': # 3
    input_data, labels_list = datasets.load_iris(return_X_y=True)
    level_one_min, level_one_max, level_one_gap = 1, 6, 1
    # 4-2, 5-3, 6-4, 7-5, 8-5, 9-5, 10-5, 11-5, 12-5, 13-5, 14-5
    ...
    data_x_and_y = np.loadtxt(fname, delimiter='\t')  # delimiter参数依据原始文本数据每行数字之间符号，这里为\t
    print('data_x_and_y:', type(data_x_and_y), data_x_and_y.shape, data_x_and_y)
    input_data, labels = np.split(data_x_and_y, [7], axis=1)  # data为拆分的数组，[1]为拆分的行号或列号，axis表明按列或者行进行拆分（默认为0，即按行拆分）
    # print('input_data:', type(input_data), input_data.shape, input_data)
    # print('labels:', type(labels), labels.shape, labels)
    # level_one_min, level_one_max, level_one_gap = 1, 7, 1
    # 3-1, 4-2, 5-2, 6-2, 7-2, 8-2, 9-2, 10-8
    labels_list = []
    for label_list in labels.tolist():
        labels_list.append(label_list[0])
    cluster_num = len(list(set(labels_list)))

elif dataset == 'sonar':  # 2
    fname = '../file/real_dataset/sonar.all-data'
    print('load :', fname)
    df = pd.read_csv(fname, header=None, error_bad_lines=False)
    df = df.to_numpy()
    print('df:', type(df), df.shape, df)
    x_cols, y_cols = np.split(df, [60], axis=1)
    # print('x_cols:', type(x_cols), x_cols.shape, x_cols)
    # print('y_cols:', type(y_cols), y_cols.shape, y_cols)
    input_data = x_cols
    # level_one_min, level_one_max, level_one_gap = 1, 5, 1
    # 3-1, 4-2, 5-2, 6-4, 7-4, 8-4, 9-7, 10-7
    labels_list = []
    for label_list in y_cols.tolist():
        labels_list.append(label_list[0])
    # print('labels_list:', type(labels_list), len(labels_list), labels_list)
    cluster_num = len(list(set(labels_list)))
    ...
    # level_one_min, level_one_max, level_one_gap = 1, 6, 1
    # 4-2, 5-3, 6-4, 7-4, 8-4, 9-4, 10-4, 11-4, 12-4, 13-10, 14-10
    cluster_num = len(list(set(labels_list.tolist())))

elif dataset == 'olivetti_faces': # 40
    input_data, labels_list = datasets.fetch_olivetti_faces(return_X_y=True)
    cluster_num = len(list(set(labels_list.tolist())))

elif dataset == 'miceprotein': # 8
    # input_data, labels_list = datasets.fetch_lfw_pairs(return_X_y=True)
    mice = datasets.fetch_openml(name='miceprotein', version=4)
    # input_data, labels_list = mice['data'], mice['target']
    ...
    # input_data = np.zeros(shape=(x_cols.shape[0], x_cols.shape[1]))
    # labels_list = []
    # col_num = 0
    # for i in range(x_cols.shape[0]):
    #     num = 0
    ...
    data_x = np.loadtxt(fname_x, delimiter=' ')  # delimiter参数依据原始文本数据每行数字之间符号，这里为\t
    data_y = np.loadtxt(fname_y, delimiter='\t')  # delimiter参数依据原始文本数据每行数字之间符号，这里为\t
    # print('data_x:', type(data_x), data_x.shape, data_x)
    # print('data_y:', type(data_y), data_y.shape, data_y)
    ...
    # 10-8, 11-9, 12-9, 13-9, 14-12, 15-12, 16-12, 17-15, 18-15, 19-15, 20-15
    cluster_num = len(list(set(labels_list.tolist())))



# (n_samples, n_features), n_digits = input_data.shape, np.unique(labels_list).size
...
# level_one_min, level_one_max, level_one_gap = 1, int(np.floor(input_data.shape[0] ** 0.5) / 2)+1, 1 # for colon cancer for LL
